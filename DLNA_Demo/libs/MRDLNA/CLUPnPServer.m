...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didSendDataWithTag:(long)tag{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(weakSelf.searchTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        weakSelf.receiveDevice = NO; //没有搜索
        CLLog(@"搜索结束");
        if(weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(didEndSearch)]){
            [weakSelf.delegate didEndSearch];
        }
    });
}
...
//通过udpSocket发送的udp搜索请求 返回来了数据data
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didReceiveData:(NSData *)data
      fromAddress:(NSData *)address
withFilterContext:(nullable id)filterContext{
    [self JudgeDeviceWithData:data];// 判断设备
}
...
- (void)JudgeDeviceWithData:(NSData *)data{
    @autoreleasepool {
        ...
        if ([string hasPrefix:@"NOTIFY"]) {
        ...
        }else if ([string hasPrefix:@"HTTP/1.1"]){ //收到了UDP端口返回的响应消息格式是HTTP/1.1 200 OK ...
            NSString *location = [self headerValueForKey:@"Location:" inData:string]; //拿到 location的值（字符串处理）
            NSString *usn = [self headerValueForKey:@"USN:" inData:string];//拿到 usn的值（字符串处理）
            if ([self isNilString:usn]) {
                CLLog(@"usn = nil");
                return;
            }
            ...
            //异步处理收到的设备信息
            dispatch_async(_queue, ^{
                if ([self.deviceDictionary objectForKey:usn] == nil) //如果设备字典里面没有
                {
                    //添加字典对象  usn为key 、CLUPnPDevice对象为value
                    [self addDevice:[self getDeviceWithLocation:location withUSN:usn] forUSN:usn];
                }
            });
        }
...
//异步主线程来通知代理  onChange了 传出所有的设备对象数组（array(CLUPnPDevice对象数组)）
- (void)onChange{
...
//通过访问location 来创建一个CLUPnPDevice对象出来（给对象赋值需要的信息） usn作为对象的uuid
